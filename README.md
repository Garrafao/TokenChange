...
`Files/Clustering/cluster_labels.csv` For storing all cluster labels.
`Files/Clustering/cluster_scores.csv` For storing all cluster performance scores.
`Files/LSC/lsc_scores.csv` For storing all LSC scores.
`Files/LSC/FirstOrder/matrix.npz` For storing count-based type vectors.
`Files/LSC/FirstOrder/w2i.npz.npy` For storing the word-to-index dictionary. 
`Files/LSC/SecondOrder/Vectors.npz` For storing the token vectors. 
